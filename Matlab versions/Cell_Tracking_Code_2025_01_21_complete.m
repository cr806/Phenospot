...
if analyse_all_wells == 1
    clear all
    close all
    HyS_path = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_1\HyperSpectral\'; % this is where the hyperspectral data is saved in the folders 1,2,3,...
    root = HyS_path;
    PhC_path = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_1\PhaseContrast\'; %This path has to be where the phase contrast images were saved
    savepath = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_1\Output\'; %This can be any path where you want to save the final histogram workspaces for further processing, not the 2 above
    scriptpath = 'P:\Phenospot Data\Sensor Tests\Drift testing\scripts\';
    cellCoordinatesPath = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_1\CellCoordinates\';
    cellCoordinatesFile = 'Refined Cell Coordinates 03102024 Location 1.xlsx';
    analyse_all_wells = 1;
    number_of_wells = 1;
...
if analyse_all_wells == 2
    clear all
    close all
    HyS_path = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_2\HyperSpectral\'; % this is where the hyperspectral data is saved in the folders 1,2,3,...
    root = HyS_path;
    PhC_path = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_2\PhaseContrast\'; %This path has to be where the phase contrast images were saved
    savepath = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_2\Output\'; %This can be any path where you want to save the final histogram workspaces for further processing, not the 2 above
    scriptpath = 'P:\Phenospot Data\Sensor Tests\Drift testing\scripts\';
    cellCoordinatesPath = 'D:\Alasdair\Phenospot data\2024-10-03\Main Expt - Copy\Location_2\CellCoordinates\';
    cellCoordinatesFile = 'Refined Cell Coordinates 03102024 Location 2.xlsx';
    analyse_all_wells = 2;
    number_of_wells = 2;
end
...
%% Sequential Renumbering of cellId in newTable so it starts at 1
%% Extract unique cell IDs from the current newTable
uniqueCellIds = unique(newTable.cellId);
%% Create a mapping from the original cell IDs to sequential IDs
cellIdMap = containers.Map(uniqueCellIds, 1:numel(uniqueCellIds));
%% Update the cellId column in newTable to sequential numbers
newTable.cellId = arrayfun(@(x) cellIdMap(x), newTable.cellId);
...
%% 2024-10-21 set ar to whole image as well -- Isabel's code - not sure what ar is for, doesn't appear to be used anywhere?
...
%        F2(d-ini+1) = getframe(fig2);    
        ...
        cd(savepath)
        open(v2)
            F2Frame = getframe(fig2);
            writeVideo(v2,F2Frame)
        close(v2)
        clear F2Frame;
...
%    cellId = uniqueCellIds(n);
    cellId = n;
%% I think I put this in because cellId started at zero, but I have changed it to start at 1 so removing
%    cellId = cellId + 1;
...
%        cellMarker{n} = [num2str(n) '_' num2str(frameNum)];
...
%% debug code output to screen to allow progress checking        
        if n == 1
            loopNote = 0;
        end
        loopNote = loopNote + 1;
...
for cellIdx = 1:numCells
    ...
    v3 = VideoWriter(cell_video_name, 'MPEG-4');
...
%        fig8.Position(1:4) = [100 200 1000 600];
        fig8.Position(1:4) = [100 200 800 600];
            subplot(2,3,1)
                imshow(mapregion, [])
            ...
            subplot(2,3,3) 
                ...
                colormap(gca,jet)
                colorbar
                text_3 = [num2str(t_interval(frameIdx),'%0.1f') ' h   '];
...
% 
            ...
            F8Frame = getframe(fig8); % Capture the updated figure (single frame)
            writeVideo(v3, F8Frame); % Write the single frame
